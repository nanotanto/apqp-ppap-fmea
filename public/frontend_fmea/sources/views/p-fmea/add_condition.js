		...
		return {
            ...
            body:{
                type:"wide", rows:[
                    ...
                    {
            cols:[
                { id:"mode_name","view": "text", "label": "Failure Mode (FM)", "name": "mode",readonly:true, gravity:3, labelPosition:"top" },
                { id:"severity","view": "text", "label": "Severity", "name": "s",readonly:true, format:"111", labelPosition:"top" },
            ]
        },
                ...
                { id:"btn_save_condition","view": "button", "css": "webix_primary", "label": "Save",
                    click:()=>{                                               
                        ...
                        webix.confirm("Do you wont to save data ?").then(function(result){
                            if (data.ap == 'High') {
                                webix.ajax().get("highLevel/"+id);
                            };

                            webix.ajax().
                            headers({'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}).
                            post("currents/save", data).then(() => {
                                webix.message("Saved")
